//=============================================================================
// SkAiKnownLists.mjs — server-side catalogs for long paste_grid lists (bypass Qwen JSON)
//=============================================================================
...
export const FRENCH_DEPARTMENTS = {
  ...
  headers: ['Code', 'Département', 'Chef-lieu', 'Région'],
  rows: [
...
export const FRENCH_REGIONS = {
  ...
  rows: [
    ...
    ['94', 'Corse', 'Ajaccio'],
    ['01', 'Guadeloupe', 'Basse-Terre'],
    ['02', 'Martinique', 'Fort-de-France'],
    ['03', 'Guyane', 'Cayenne'],
    ['04', 'La Réunion', 'Saint-Denis'],
    ['06', 'Mayotte', 'Mamoudzou'],
  ],
};

/**
 * Match a built-in catalog from the user prompt (not a capability question).
 * @param {string} prompt
 * @returns {SkKnownList|null}
 */
export function matchKnownList(prompt) {
  const wText = String(prompt || '').trim();
  if (!wText) {
    return null;
  }
  const wFr = /\bfran[cç]/i.test(wText);
  if (/\bd[ée]partements?\b/i.test(wText) && (wFr || /\bliste\b/i.test(wText))) {
    return FRENCH_DEPARTMENTS;
  }
  if (/\br[ée]gions?\b/i.test(wText) && (wFr || /\bliste\b/i.test(wText))) {
...
/**
 * @param {SkKnownList} list
 * @returns {Array<Array<string|number>>}
 */
export function buildKnownListGrid(list) {
